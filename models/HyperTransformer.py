...
from models.Attention_test import get_encoder_reference_points
...
class SFE(nn.Module):
    def __init__(self, in_feats, num_res_blocks, n_feats, res_scale):
        ...
        self.conv_head = conv3x3(in_feats, n_feats)  # spectral_nums :4

        self.RBs = nn.ModuleList()
        for i in range(self.num_res_blocks):
            self.RBs.append(ResBlock(in_channels=n_feats, out_channels=n_feats,  # 256  256*64*64
                                     res_scale=res_scale))
...
class ResNet(nn.Module):
    ...
    def __init__(self, layers: List[int], num_classess=1000, in_feats=4,
                 num_res_blocks=16, n_feats=256, res_scale=1):
        super(ResNet, self).__init__()
        self.in_channels = 64
        self.bottleneck = Bottleneck
        # The followling layers define stage 1(befor residual blocks)
        self.conv0_1 = nn.Conv2d(in_feats, self.in_channels, kernel_size=1,
                                 stride=1, padding=0, bias=False)
        #self.conv0_2 = nn.Conv2d(self.in_channels, self.in_channels, kernel_size=1,
        #                         stride=1, padding=0, bias=False)
        self.conv0_3 = nn.Conv2d(self.in_channels, 3, kernel_size=1,
                                 stride=1, padding=0, bias=False)

        self.conv1 = nn.Conv2d(3, self.in_channels, kernel_size=7,
                               stride=1, padding=3, bias=False).requires_grad_(False)
        self.bn1 = nn.BatchNorm2d(self.in_channels).requires_grad_(False)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1).requires_grad_(False)
        # The following layers define stage 2-5(residual blocks)
        self.layer1 = self._make_layer(64, layers[0])  # 3
        self.layer2 = self._make_layer(128, layers[1], stride=1)  # 4
        ...
        self.conv_out1 = nn.Conv2d(512, n_feats, kernel_size=1,
                                   stride=1, padding=0, bias=False)
        self.conv_out2 = nn.Conv2d(n_feats, n_feats, kernel_size=1,
                                   stride=1, padding=0, bias=False)
        self.conv_out3 = nn.Conv2d(n_feats, n_feats, kernel_size=1,
                                   stride=1, padding=0, bias=False)

    # Build residual block
    def _make_layer(self, channels, num_bottleneck, stride=1):  # 64 4
        ...
        return nn.Sequential(*layers).requires_grad_(False)

    def forward(self, x):
        # Stage 1
        adapter0 = self.conv0_1(x)
        #adapter0 = self.conv0_2(adapter0)
        adapter0 = self.conv0_3(adapter0)

        # with torch.no_grad():
        with torch.no_grad():
            out = self.maxpool(self.relu(self.bn1(self.conv1(adapter0))))
            # print(out.shape)
            # Stage 2-5
            out = self.layer1(out)
            out = self.layer2(out)

        adapter = self.conv_out1(out)
        out = self.conv_out2(adapter)
        out = self.conv_out3(out) + adapter
        # out = self.layer3(out)
        # out = self.layer4(out)
        # # GlobalAvgPool-FC
    ...
    # Construct ResNet-n
    def ResNet50(num_classess, in_feats=4, n_feats=256):
        return ResNet([3, 4, 6, 3], num_classess, in_feats=in_feats,
                      n_feats=n_feats)
...
class HyperTransformerPre(nn.Module):
    def __init__(self, config):
        ...
        # self.TS_lv2 = ScaledDotProductAttentionOnly(temperature=lv2_dim)
        # self.TS_lv1 = ScaledDotProductAttentionOnly(temperature=lv3_dim)
        self.TS_lv3 = MultiScaleDeformableAttention(embed_dims=256,num_levels=1, num_heads=4, batch_first=True)
        self.TS_lv2 = MultiScaleDeformableAttention(embed_dims=128,num_levels=1, num_heads=4, batch_first=True)
        self.TS_lv1 = MultiScaleDeformableAttention(embed_dims=64,num_levels=1, num_heads=4, batch_first=True)
        ...
        state_dict = torch.hub.load_state_dict_from_url(pretraind_model_urls['resnet50'], progress=True)
        state_dict['conv0_1.weight'] = torch.randn([64, self.in_channels, 1, 1])
        # state_dict['conv0_1.bias'] = torch.randn([self.in_channels])
        #state_dict['conv0_2.weight'] = torch.randn([64, 64, 1, 1])
        # state_dict['conv0_2.bias'] = torch.randn([self.in_channels])
        state_dict['conv0_3.weight'] = torch.randn([3,64, 1, 1])
        # state_dict['conv0_3.bias'] = torch.randn([3])

        state_dict['conv_out1.weight'] = torch.randn([ self.n_feats,512, 1, 1])
        # state_dict['conv_out1.bias'] = torch.randn([self.n_feats])
        state_dict['conv_out2.weight'] = torch.randn([self.n_feats, self.n_feats, 1, 1])
        # state_dict['conv_out2.bias'] = torch.randn([self.n_feats])
        state_dict['conv_out3.weight'] = torch.randn([self.n_feats, self.n_feats, 1, 1])
        # state_dict['conv_out3.bias'] = torch.randn([self.n_feats])

        self.SFE.load_state_dict(state_dict)
        ###############
        ### stage11 ###
        ###############
        self.conv11_head = conv3x3(2 * self.n_feats, self.n_feats)
    ...
    def forward(self, X_MS, X_PAN):
        ...
        T_lv1 = self.TS_lv1(Q_lv1.view(b_1,  w_1 * h_1, c_1),K_lv1.view(b_1,  w_1 * h_1, c_1),
                            V_lv1.view(b_1,  w_1 * h_1, c_1),spatial_shapes=torch.tensor([[w_1, h_1]]).to(Q_lv1.device),
                             level_start_index=torch.tensor([0, w_1 * h_1]).to(Q_lv1.device),
                             reference_points=reference_point_1)
        T_lv2 = self.TS_lv2(Q_lv2.view(b_2,  w_2 * h_2, c_2),K_lv2.view(b_2,  w_2 * h_2, c_2),
                            V_lv2.view(b_2,  w_2 * h_2, c_2),spatial_shapes=torch.tensor([[w_2, h_2]]).to(Q_lv1.device),
                             level_start_index=torch.tensor([0, w_2 * h_2]).to(Q_lv1.device),
                             reference_points=reference_point_2)
        T_lv3 = self.TS_lv3(Q_lv3.view(b_3,  w_3 * h_3, c_3),K_lv3.view(b_3,  w_3 * h_3, c_3),
                            V_lv3.view(b_3,  w_3 * h_3, c_3),spatial_shapes=torch.tensor([[w_3, h_3]]).to(Q_lv1.device),
                             level_start_index=torch.tensor([0, w_3 * h_3]).to(Q_lv1.device),
                             reference_points=reference_point_3)
        T_lv1 = T_lv1.view(b_1,c_1,h_1,w_1)
        T_lv2 = T_lv2.view(b_2,c_2,h_2,w_2)
        T_lv3 = T_lv3.view(b_3,c_3,h_3,w_3)
        # Shallow Feature Extraction (SFE)
        x = self.SFE(X_MS)  # 输入 4 16 16 输出 256 16 16
